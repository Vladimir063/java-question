...
Объединённая точка (Join point) - Это такая точка в приложении, где мы можем подключить аспект. Другими словами, это место, где начинаются определённые действия модуля АОП в Spring. Это момент в программе, где может быть выполнен дополнительный код (advice). По сути, join point — это точка потенциального вмешательства, например: вызов метода userService.saveUser() или orderService.createOrder().
...
+ @ResponseBody - позволяет отправлять Object в ответе. Обычно используется для отправки данных формата XML или JSON. «Возвращаемое значение метода нужно сериализовать в тело HTTP-ответа».
...
# Как работает Propagation в Spring под капотом (понятно и по шагам)

Аннотация `@Transactional` в Spring — это просто **сигнал прокси**, что метод должен выполняться в определённом **транзакционном контексте**.  
То, как именно создаются или переиспользуются транзакции, решается не базой данных, а **Spring AOP и TransactionManager**.

---

## 1️⃣ Прокси и перехват вызова

Когда контейнер видит аннотацию `@Transactional`, он **оборачивает бин в прокси**.  
Если это интерфейс — используется JDK Dynamic Proxy, если класс — CGLIB.

Когда вызывается метод, Spring **перехватывает вызов** и запускает специальный класс — `TransactionInterceptor`.

---

## 2️⃣ TransactionInterceptor — сердце механизма

`TransactionInterceptor` делает примерно следующее:

1. Извлекает настройки аннотации (`@Transactional`):  
   ```java
   propagation = REQUIRED;
   isolation = DEFAULT;
   readOnly = false;
   ```
2. Передаёт их в `TransactionAspectSupport.invokeWithinTransaction()` — основной метод, управляющий транзакцией.
3. Этот метод решает: использовать ли уже существующую транзакцию, создать новую, приостановить или бросить исключение.

---

## 3️⃣ Откуда Spring знает, есть ли уже транзакция

Spring хранит состояние текущей транзакции в **ThreadLocal**, через класс `TransactionSynchronizationManager`.

Когда начинается новая транзакция, туда записывается объект соединения (например, `ConnectionHolder`) и статус активности.  
При последующих вызовах Spring просто проверяет этот контекст:
```java
TransactionSynchronizationManager.isActualTransactionActive();
```

Если метод с `@Transactional` вызывается из другого транзакционного метода **в том же потоке**, Spring видит активную транзакцию и реагирует в зависимости от политики Propagation.

---

## 4️⃣ Что делает каждый Propagation под капотом

| Propagation | Что делает | Внутреннее действие |
|--------------|-------------|----------------------|
| **REQUIRED** | Использует текущую транзакцию, если она есть, иначе создаёт новую | Проверяет ThreadLocal → если пусто → `transactionManager.getTransaction()` |
| **REQUIRES_NEW** | Всегда начинает новую транзакцию | Приостанавливает текущую (`suspend()`), открывает новую |
| **SUPPORTS** | Если транзакция есть — участвует, если нет — работает без неё | Просто выполняет метод, не создавая транзакцию |
| **NOT_SUPPORTED** | Всегда выполняется без транзакции | Приостанавливает текущую, потом возобновляет |
| **MANDATORY** | Требует существующей транзакции | Если нет активной — бросает `TransactionRequiredException` |
| **NEVER** | Запрещает транзакции | Если активная есть — бросает исключение |
| **NESTED** | Создает "подтранзакцию" (savepoint) | Использует JDBC savepoint через `DataSourceTransactionManager` |

---

## 5️⃣ Что делает TransactionManager

Когда Spring решает начать или присоединиться к транзакции, он делегирует всё в `PlatformTransactionManager`, чаще всего — `DataSourceTransactionManager`.

Под капотом:
1. Получает JDBC-соединение из DataSource.
2. Вызывает `setAutoCommit(false)`.
3. Создаёт объект `TransactionStatus` — это обёртка с флагами (isNewTransaction, rollbackOnly и т.д.).
4. Сохраняет контекст транзакции в `ThreadLocal`.

---

## 6️⃣ Коммит и откат

После выполнения метода Spring делает:
```java
try {
...
- Если в коде вызвано `TransactionAspectSupport.currentTransactionStatus().setRollbackOnly()`, то коммит заменяется на откат.

---

## 7️⃣ Роль TransactionSynchronizationManager

Этот класс держит в `ThreadLocal`:
- текущую транзакцию;
- связанные ресурсы (Connection, Session);
- список колбэков (Synchronization) для commit/rollback событий.

Благодаря этому Spring "понимает", находится ли код уже внутри транзакции, и может принимать решения по propagation.

---

## 🧠 Итого: как всё работает по шагам

1. Прокси перехватывает метод.
2. TransactionInterceptor читает аннотацию `@Transactional`.
3. TransactionAspectSupport проверяет активность транзакции.
4. В зависимости от Propagation:
   - создаётся новая транзакция;
   - используется текущая;
   - приостанавливается текущая;
   - или выбрасывается исключение.
5. TransactionManager управляет JDBC-соединением.
6. Commit или rollback выполняются после завершения метода.
7. Всё состояние хранится в `ThreadLocal`.

---

Таким образом, **Propagation** — это просто **набор правил**, который говорит Spring,  
**как себя вести при входе в метод с аннотацией `@Transactional`, если уже существует транзакция**.

---

📚 Основные классы, участвующие в механизме:
...
- `TransactionDefinition`
  ...
  # Прокси при транзакциях в Spring: JDK Dynamic Proxy vs CGLIB

Когда вы ставите `@Transactional`, вызов метода проходит через **AOP‑прокси**, который подключает `TransactionInterceptor`. Тип прокси влияет **только на способ перехвата вызова**, но не на саму транзакционную семантику (propagation, rollback и т.п.). В Spring используются два механизма:

---

## 1) JDK Dynamic Proxy (интерфейсный)
**Что это:** создаёт прокси, который реализует **интерфейсы** вашего бина и делегирует вызовы реальному объекту.

**Как работает с транзакциями:** вызовы методов через интерфейс попадают в прокси → `TransactionInterceptor` решает открыть/присоединиться/приостановить транзакцию → выполняет метод → commit/rollback.

**Плюсы**
- Простая и «лёгкая» технология стандартной JDK (без внешних зависимостей).
- Не требует наследования от класса (подходит для «закрытых» финальных классов, если у них есть интерфейсы).
- Чуть меньше накладных расходов на генерацию класса.

**Минусы**
- Перехватывает **только методы, объявленные в интерфейсах**. Вызовы методов, которых нет в интерфейсе (или обращение к ним через ссылку на класс), не будут проходить через прокси.
- Если клиент вызывает метод напрямую по типу класса (а не по интерфейсу), перехвата не будет.
- Те же ограничения Spring AOP: **self‑invocation** (внутренний вызов метода того же бина) не перехватывается.

**Когда выбирать**
- Бин **экспонирует интерфейс** и вы обращаетесь к нему через интерфейс.
- Хотите минимальные зависимости и проще понимать контракт бина.
- Не нужно перехватывать методы, отсутствующие в интерфейсе.

---

## 2) CGLIB (class‑based)
**Что это:** создаёт **подкласс** вашего бина (через bytecode‑generation) и переопределяет методы, чтобы вставить перехватчик.

**Как работает с транзакциями:** вызовы публичных/защищённых (не final) методов на прокси‑экземпляре идут в переопределённые методы подкласса → `TransactionInterceptor` → дальше как обычно.

**Плюсы**
- Позволяет перехватывать **методы класса**, даже если они не объявлены в интерфейсе.
- Удобно, когда у бина **нет интерфейса**.

**Минусы**
- **Нельзя проксировать final‑классы и final‑методы** (их нельзя переопределить) — такие вызовы не будут перехвачены.
- Немного тяжелее по накладным расходам (генерация подкласса).
- Те же ограничения Spring AOP: **self‑invocation** внутри экземпляра не перехватывается.

**Когда выбирать**
- У бина **нет интерфейсов**, а транзакции/AOP всё равно нужны.
- Нужно перехватывать методы, **не объявленные в интерфейсе**.
- Ок с ограничением «без final» (или можете сделать методы/класс открытыми). Для Kotlin: используйте `kotlin-spring`/`all-open` плагины, чтобы делать классы/методы `open`.

---

## Что и как Spring выбирает по умолчанию
- **Spring Framework (без Boot):** по умолчанию использует **JDK Dynamic Proxy**, если бин реализует **хотя бы один интерфейс**; иначе — падает обратно на **CGLIB**.
- Это поведение можно **переопределить** и *насильно* включить CGLIB, даже если есть интерфейсы:
  - `@EnableTransactionManagement(proxyTargetClass = true)`  
  - или `@EnableAspectJAutoProxy(proxyTargetClass = true)`  
  - или в Boot: `spring.aop.proxy-target-class=true` (в `application.properties`/`yaml`).

> Итог: выбор — **семантический**, а не «про производительность». Смотрите на форму API бина (интерфейсы/классы) и на то, какие методы должны перехватываться.

---

## Важные нюансы для транзакций (независимо от типа прокси)
- **Self‑invocation**: если метод A внутри того же бина вызывает метод B с `@Transactional`, перехвата не будет (вызов идёт минуя прокси). Решения:
  - вынести B в другой бин; или
  - использовать AspectJ LTW/CTW (не прокси‑AOP); или
  - вызывать через прокси: получить его из контекста и вызывать.
- **Модификаторы**: Spring AOP перехватывает **public/protected** методы. Private не перехватываются.
- **Final** (только для CGLIB): final‑методы/классы — не перехватываются; для Kotlin делайте `open`.
- **Контракт интерфейса** (только для JDK): методы, которых нет в интерфейсе, не через прокси → транзакция не сработает.
- **Вызовы из другого бина всегда идут через прокси**, если инъекция получена из контейнера.
- **Производительность**: современные JDK и CGLIB делают разницу минимальной. Делайте выбор по **семантике и ограничениям**, а не по micro‑benchmark’ам.

---

## Быстрые рекомендации
- Есть интерфейс и вы работаете через него → **JDK Dynamic Proxy** (по умолчанию).
- Нет интерфейсов или нужно перехватывать все методы класса → **CGLIB** (включите `proxyTargetClass=true`).
- Kotlin/финальные классы → либо интерфейсы, либо `open`‑классы/методы.
- Важно перехватить внутренние вызовы → рассмотрите **AspectJ (compile‑time / load‑time weaving)**.

---

## Примеры включения

**По умолчанию (интерфейс → JDK‑прокси):**
```java
...
spring:
  aop:
    proxy-target-class: true   # форсируем CGLIB
```

---

### TL;DR
- **JDK Proxy** → перехватывает вызовы **через интерфейс**. Простой и безопасный выбор, если у бина есть интерфейс.
- **CGLIB** → перехватывает **методы класса**, но не работает с `final`. Нужен, когда интерфейсов нет или нужно перехватывать больше методов.
- Тип прокси не меняет логику транзакций, он лишь определяет, **пройдут ли ваши вызовы через перехватчик**.
