...
* В случае если один из потребителей выходит из строя, его партиции автоматически перераспределяются между оставшимися активными потребителями
 ...
 Consumer Group — это механизм, который позволяет координировать консумеров и распределять нагрузку между ними.
...
| --------------------------- | ----------------------------------------------------------------- | ----------------------------------------------------------- |
| 🧱 **Тип системы**          | Distributed **event streaming platform** (лог сообщений)          | Classic **message broker** (очереди)                        |
| 📦 **Модель хранения**      | Сообщения хранятся **на диске долго** (лог, partition)            | Сообщения **удаляются после доставки**                      |
| 🔄 **Подход к доставке**    | **Pull** – consumer сам запрашивает данные                        | **Push** – брокер сам отправляет consumer’у                 |
| 👥 **Модель подписчиков**   | Consumer Groups — распределение партиций между потребителями      | Очереди и подписчики — можно fanout, direct, topic exchange |
| ⚡️ **Производительность**   | Очень высокая (до миллионов сообщений/сек)                        | Ниже, оптимизирован под надёжную доставку                   |
| ⏳ **Сохранение истории**    | Да, можно перечитать старые сообщения (retention)                 | Нет, сообщение удаляется после ack                          |
| 🧩 **Обработка сообщений**  | Подходит для **потоковой обработки, аналитики, логирования**      | Подходит для **очередей задач, RPC, уведомлений**           |
| 🧠 **Порядок сообщений**    | Гарантируется **внутри партиции**                                 | Гарантируется **в очереди**                                 |
| 🧾 **Доставка сообщений**   | At-least-once / Exactly-once                                      | At-most-once / At-least-once                                |
| ⚙️ **Архитектура**          | Децентрализованная, кластер брокеров, координатор Zookeeper/KRaft | Централизованный брокер + очереди                           |
| 📊 **Хранение состояния**   | Есть (Kafka Streams, state store)                                 | Нет встроенного состояния                                   |
| 🧰 **Типичные кейсы**       | Big Data, event sourcing, log aggregation, stream processing      | Task queues, async jobs, transactional messaging            |
| 🧩 **Интеграция со Spring** | `spring-kafka`                                                    | `spring-amqp`                                               |

Итог:
Kafka → для высоконагруженных потоков событий и аналитики.
RabbitMQ → для традиционного обмена сообщениями и асинхронных очередей задач.
