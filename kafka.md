...
- Не создавать «с запасом» тысячи партиций без реальной необходимости.  
- Правило практики: **количество партиций ≈ несколько × (число консьюмеров × коэффициент роста нагрузки)**.  
- Тестировать кластер под нагрузкой до того, как уйти в прод.  
- Следить за метриками:
  - `kafka.server:type=BrokerTopicMetrics` (latency, throughput),  
  - heap usage у контроллера,  
  - время ребаланса.

---

✅ Итог: **слишком много партиций может перегрузить память, диск, сеть и метаданные в Kafka.**  
Нужно балансировать между параллелизмом и управляемостью кластера.
