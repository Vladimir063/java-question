...
# 12. Короткое резюме и практические советы для собеседования

- Знайте разницу между young/old/meta/stack и роль каждого в GC. (цитируйте примеры и команды) — это фундамент.  
- Ознакомьтесь с G1 (дефолт), ZGC и Shenandoah — умейте объяснить trade-offs (latency vs throughput vs CPU/memory).  
- Умейте читать GC логи и привести пример команды запуска и анализа (как в разделе 7).  
- На реальные кейсы — предлагайте тесты (нагрузочное тестирование), профиль памяти, heap-dump, и потом уже тюнинг JVM/GC.

---

# 13. Источники (ключевые для проверяющих факты)
- Официальная документация Oracle по G1 и общему управлению памятью.  
- OpenJDK (Shenandoah wiki, JEP для ZGC/генерационной поддержки).  
- Обучающие статьи/гайды (DigitalOcean, DZone) и сравнительные исследования для практических выводов.
