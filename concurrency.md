...
1. Действия внутри одного потока идут в порядке программы — то есть инструкции в одном методе имеют порядок (program order).
   “Program order” — это правило happens-before, которое гарантирует: внутри одного потока инструкции выполняются в том порядке, в котором они написаны, и это базовый строительный блок для понимания видимости между потоками.
   ```java
   int a = 1;      // действие 1
	int b = a + 2;  // действие 2
	System.out.println(b); // действие 3
      ```
В одном потоке гарантировано:
a = 1 выполнится перед b = a + 2
b = a + 2 выполнится перед System.out.println(b)
Даже если компилятор или процессор делает оптимизации, с точки зрения этого потока результат будет как будто инструкции выполнялись строго по порядку.

2. Запись в `volatile` переменную → чтение той же `volatile` переменной (volatile write -> volatile read) — это даёт видимость. Когда один поток записывает значение в volatile переменную, а другой поток потом читает эту же переменную, все изменения, сделанные до записи, гарантированно видны второму потоку.
3. `synchronized` блоки: `unlock` монитора в одном потоке → `lock` того же монитора в другом потоке. Если один поток выходит из synchronized блока (unlock) на каком-то мониторе, а другой поток входит в synchronized блок (lock) на том же мониторе, то: Все изменения, сделанные первым потоком внутри синхронизированного блока, гарантированно видны второму потоку.
...
5. Правила для `final` полей при корректной публикации объекта. Если объект создан и правильно опубликован (например, через static final, volatile, synchronized или final ссылку), то все final поля объекта гарантированно видны другим потокам в том виде, в котором они были инициализированы в конструкторе. Другими словами, конструктор полностью инициализирует final поля, и другие потоки не увидят их “полуготовыми”.
