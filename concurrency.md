...
Без модели памяти результат многопоточной программы был бы непредсказуем: разные процессоры, разные JVM, разное поведение.
...
|------|------------|-------------|
| `int`, `byte`, `boolean`, `char`, `short` | ✅ Да |
| `long`, `double` | ✅ С Java 5 |
| Составные операции (`i++`) | ❌ Нет |
| `volatile` запись/чтение | ✅ Да |
| `Atomic*` | ✅ Да (через CAS) |
...
| Модель | Описание |
|---------|-----------|
| **Strong (Sequential Consistency)** | Все потоки видят одинаковый порядок операций |
| **Weak (JMM)** | Порядок может отличаться, но определён барьерами и правилами |
...
| Концепция | Смысл |
|------------|-------|
| **JMM** | Спецификация взаимодействия потоков с памятью |
| **Working Memory** | Локальный кэш потока |
| **Main Memory** | Общая область (heap) |
| **Visibility** | Видимость изменений между потоками |
| **Atomicity** | Неделимость операции |
| **Reordering** | Перестановка инструкций |
| **Memory Barriers** | Ограничения порядка |
| **Publication** | Безопасное распространение объекта |
| **Final Semantics** | Гарантии инициализации final полей |

---

📘 **Итог:**
> Java Memory Model — это формальная спецификация, описывающая, как потоки читают и записывают общие данные, какие операции могут быть переупорядочены, как кэшируются переменные, и как инструменты (`volatile`, `synchronized`, `Lock`, `Atomic*`) управляют видимостью и порядком исполнения между потоками.
...
## 1) Порядок программы (program order rule)
**Что это:** внутри одного и того же потока операции ведут себя так, *как будто* выполняются в том порядке, в котором написаны.

```java
int a = 1;           // действие 1
int b = a + 2;       // действие 2
System.out.println(b); // действие 3
```

В одном потоке гарантии такие:
- `1` happens-before `2`
- `2` happens-before `3`

Компилятор/CPU вправе переставлять инструкции для оптимизаций, но наблюдаемое поведение в **этом же потоке** эквивалентно исходному порядку.   Компилятор и CPU могут переставлять независимые операции,
если это не меняет наблюдаемое поведение программы в рамках одного потока.
> Ограничение: это не создаёт видимость между *разными* потоками — для этого нужны другие правила.

---

## 2) Транзитивность (transitivity)
**Что это:** если `A` happens-before `B`, и `B` happens-before `C`, то **`A` happens-before `C`**.  
Это важнейшее свойство: многие схемы синхронизации опираются на «сшивание» нескольких ребер HB в одно.

---

# Правила видимости через синхронизацию

## 3) Volatile: запись → чтение

 ⚙️ Правило: запись в volatile → чтение volatile

**Определение:**  
Запись в `volatile` переменную в одном потоке **happens-before** последующего чтения *той же* `volatile` переменной другим потоком.

Это означает, что **все операции**, выполненные до записи в `volatile` переменную, становятся **видимыми** другому потоку, который позже прочитает эту же переменную.  
Иными словами, `volatile` не просто хранит значение — он действует как **мемори-барьер (memory fence)**, обеспечивающий порядок и видимость изменений.
...
Thread A:
  ...
  ready = true;    // volatile write (публикует все предыдущие записи)

Thread B:
  if (ready) {     // volatile read (видит все записи до write)
...
#### Поток A:
1. Выполняет обычную запись `data = 42;` — это значение может остаться в **регистре CPU** или **кэше ядра**.
2. При записи в `ready = true;`:
   - JVM вставляет **StoreStore Barrier** и **StoreLoad Barrier**.  
   - Это заставляет CPU **сбросить (flush)** все предыдущие записи (`data = 42`) из кэша в основную память.
   - После этого значение `ready=true` записывается в память **в последнюю очередь**.

#### Поток B:
1. Читает `ready` (volatile read).
   - JVM вставляет **LoadLoad Barrier** и **LoadStore Barrier**.
   - Эти барьеры заставляют CPU **сначала прочитать обновлённое значение `ready` из памяти**, а не из локального кэша.
   - Также они **обязывают** CPU обновить локальный кэш всеми изменениями, опубликованными до записи `ready=true`.

Результат:  
Поток B гарантированно увидит `data=42`, даже если `data` не volatile.

---

### 🧠 Важный момент: volatile обеспечивает *видимость*, но не *атомарность*

- Операции чтения/записи одиночного `volatile` переменной **атомарны**.
- Но составные операции (например, `count++`) **не атомарны**, даже если `count` volatile.

```java
volatile int counter = 0;

// Не атомарно!
counter++; // это три операции: read → increment → write
```

Для атомарности нужно использовать `AtomicInteger`, `synchronized` или `Lock`.

---

## ⚙️ Поведение при смешанных полях (volatile + non-volatile)
...
🧠 Здесь работает следующее правило:
> Все изменения (`x=1`, `y=2`), сделанные **до записи** в volatile-поле `ready`,  
> становятся **видимыми** всем потокам, которые **позже читают** `ready`.

- То есть volatile write публикует не только своё значение, но и **все предшествующие обычные записи** в тот же объект.
- Поток B, после `volatile read`, гарантированно увидит актуальные значения `x` и `y`, даже если они не volatile.

Это и есть основной механизм «публикации состояния через volatile».
...
## 🧩 Память, из которой читаются volatile и non-volatile

| Тип поля | Источник данных | Видимость между потоками |
|-----------|----------------|---------------------------|
| `volatile` | всегда из **основной памяти (main memory)** | гарантирована (все изменения видны) |
| обычное поле (`non-volatile`) | может из **регистров** или **кэша ядра потока** | не гарантирована без happens-before |

### Таким образом:
- volatile всегда читает/пишет напрямую из main memory (Heap).
- non-volatile может храниться в локальном кэше и быть «устаревшим».
- volatile обеспечивает **синхронизацию памяти** (memory synchronization point).

---

## 🔄 Влияет ли порядок объявления полей в классе?

Нет. Порядок объявления полей (`volatile` сверху или снизу) **не влияет** на поведение JMM.  
JMM не использует физический порядок полей в классе для определения барьеров.  

Все гарантии определяются **моментом чтения и записи**, а не позицией поля в классе.
...
Поведение при таких последовательностях абсолютно одинаковое:  
- `ready = true;` всё равно опубликует предшествующие записи в `x`.
- `if (ready)` всё равно синхронизирует и обновит `x` из памяти.

> ⚠️ То есть JMM ориентируется на **операции чтения/записи**,  
> а не на **физический порядок полей в байткоде**.

---

## 🧱 Мини-резюме

| Что делает `volatile` | Объяснение |
|------------------------|-------------|
| Гарантирует **видимость** между потоками | изменения до записи в volatile видны после чтения |
| Вставляет **memory barriers** | StoreLoad, StoreStore, LoadLoad, LoadStore |
| **Не гарантирует атомарность** составных операций | `volatile count++` не потокобезопасен |
| Может использоваться для **публикации состояния** | `ready=true` публикует предшествующие записи |
| Порядок объявления полей не влияет | JMM смотрит на операции, не на физический порядок |

---

## 💡 Итог

> `volatile` — это не просто «модификатор видимости».  
> Это механизм, который создаёт **точку синхронизации памяти** между потоками.  
> Он гарантирует, что все записи, сделанные до volatile write,  
> будут видимы всем потокам, которые позже делают volatile read.  
>  
> При этом обычные поля (non-volatile) читаются из локальной памяти,  
> и только операции с volatile «сбрасывают» или «обновляют» эти кэши.



---

## 4) Мониторы: unlock → lock (synchronized)
**Правило:** выход из `synchronized` (unlock монитора) **happens-before** последующим входам (lock) в *тот же* монитор другим потоком.
...
Thread A:
  synchronized (lock) {
    x = 1; // изменения внутри блока
  } // unlock: публикует изменения

Thread B:
  synchronized (lock) { // lock: видит публикации
    System.out.println(x); // гарантированно 1
  }
```

- Мониторы дают **взаимоисключение** + **видимость**: перед выходом все записи «сбрасываются» в память, перед входом — «загружаются».
- Аналогичные гарантии дают `java.util.concurrent.locks.Lock` (`unlock` → `lock`).

---

## 5) Старт и завершение потоков
**Правила:**
- `Thread.start()` **happens-before** *любыми* действиями, выполняемыми этим потоком.
- Все действия потока **happens-before** возвращением `Thread.join()` в другом потоке.

```java
public class ThreadHBExample {
    static int data = 0;
    static boolean ready = false;

    public static void main(String[] args) throws InterruptedException {

        // 👇 Поток t будет читать общие переменные
        Thread t = new Thread(() -> {
            // (3) Всё, что произошло ДО t.start() в главном потоке,
            //     становится видимо здесь — благодаря правилу start-happens-before.
            if (ready) {
                System.out.println("Thread sees: data = " + data);
            } else {
                System.out.println("Thread sees: not ready");
            }

            // (4) Поток что-то меняет
            data = 42;
            ready = true;
        ...
        // (1) Эти действия выполняются ДО запуска потока
        data = 10;
        ready = true;
        ...
        // (5) Ждём завершения потока — join()
        t.join();

        // (6) После join() гарантируется happens-before:
        //     всё, что сделал поток t, теперь видимо в основном потоке.
        System.out.println("Main sees after join: data = " + data + ", ready = " + ready);
    }
}

```

> Практика: это значит, что подготовка данных до `start()` видна потоку-работнику, а результаты его работы видны после `join()`.

---

## 6) Правила для `final` полей и «безопасная публикация»
**Инициализация `final`:** записи `final`-полей, выполненные в **завершившемся конструкторе**, становятся *навсегда видимы* другим потокам при **корректной публикации** ссылки на объект.

Корректная публикация — это передача ссылки через:
- `volatile` поле,
- `final` поле,
- `synchronized` (после unlock),
- неизменяемые контейнеры/однопоточный инициализатор (static init),
- другие механизмы, обеспечивающие HB (например, поместить в `ConcurrentHashMap`).

```java
class Holder {
  final int n;
  Holder(int n) { this.n = n; } // при условии что 'this' не «утёк» из конструктора
}

volatile Holder h;

Thread A:
  h = new Holder(42); // публикация через volatile

Thread B:
  Holder local = h;   // чтение volatile
  if (local != null) {
    // гарантированно увидит n == 42
  }
}
```

> Важная тонкость: **нельзя допускать утечки `this` из конструктора** (например, публикация в статическое поле, регистрация listener’а, запуск потока из конструктора и т.п.) — иначе гарантия видимости `final` может нарушиться.

---

# Дополнительные, но часто проверяемые правила

## 7) Прерывания: `interrupt()` → обнаружение прерывания
Вызов `Thread.interrupt()` **happens-before** тем моментом, когда целевой поток:
- выходит из блокирующего вызова, бросив `InterruptedException`, или
- наблюдает флаг через `Thread.interrupted()` / `isInterrupted()`.

Это значит, что «прерывание» и его последствия будут корректно видны.

---

## 8) Инициализация класса (class initialization)
Завершение инициализации класса/интерфейса (static инициализатор) **happens-before** *любым* использованием этого класса *любыми* потоками (создание экземпляра, обращение к static-полю, вызов static-метода).  
Отсюда следует «инициализационная безопасность» для `static final` полей.

---

## 9) Финализация (историческое, но знать полезно)
Вызов конструктора **happens-before** началом `finalize()`. (Финализация как техника считается устаревшей, но правило в модели определено.)


---

## Мини-пример, «сшивание» правил

```java
class Box { final int v; Box(int v){ this.v = v; } }

volatile Box box;

Thread A:
  Box b = new Box(42); // завершённый конструктор: final-гарантии для b.v
  box = b;             // volatile write — публикация (A → B)

Thread B:
  Box b2 = box;        // volatile read — видимость (A → B)
  if (b2 != null) {
    // благодаря final + корректной публикации,
    // здесь гарантированно увидим b2.v == 42
  }
}
```

Здесь соединяются:
- завершающийся конструктор (`final`-семантика),
- публикация через `volatile` (write→read),
...
CompletableFuture<V> — это развитие концепции Future, предоставляющее гораздо большие возможности для работы с асинхронными задачами. Он не только предоставляет результат, но и позволяет строить цепочки асинхронных операций, обрабатывать их результаты и ошибки, а также сочетать несколько задач вместе.

Некоторые ключевые особенности:

Композиция задач: Возможность объединить задачи с помощью операторов, таких как thenApply, thenCombine, whenComplete, и т.д.
Обратный вызов: Позволяет зарегистрировать колбеки, которые будут выполнены, когда задача завершится.
Ручное завершение: Позволяет установить результат или ошибку вручную.
Обработка ошибок: Гибкая интеграция с обработкой исключений.

[к оглавлению](#Многопоточность)

##  Что внутри аннотации `@Async`
`@Async` — маркер, говорящий, что метод должен выполняться асинхронно. Внутри: метаданные о том, что запускать в отдельном потоке/пуле. Обычно поддерживаются опции (необязательные): имя/идентификатор исполнителя (executor), возврат `Future`/`CompletableFuture`/`void` и обработка исключений. Фактическая логика — прокси/аспект, который делегирует вызов в пул потоков.

##  Потокобезопасный ли singleton
Сам по себе экземпляр singleton'а **не гарантирует** потокобезопасность. Потокобезопасность зависит от внутренней реализации: если поля изменяемые — нужно синхронизировать доступ, использовать `volatile`, `Atomic*` или проектировать immutable.
