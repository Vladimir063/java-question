...
# Правило happens-before для volatile и поведение при смешанных полях
...
/* действия до start() */    // видны внутри потока t
t.start();                   // HB к действиям в t
t.join();                    // всё, что сделал t, HB к коду после join()
```
