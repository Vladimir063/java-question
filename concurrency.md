...
**Правило:** запись в `volatile`-переменную **happens-before** последующего чтения этой же `volatile`-переменной другим потоком.
...
- Запись в `ready` **публикует** все предыдущие изменения (включая `data = 42`).
- Чтение `ready` **загружает** эти изменения, обеспечивая видимость.
- Дополнительно `volatile` запрещает определённые перестановки вокруг себя (memory barriers).

> Важно: `volatile` не делает составные операции атомарными (например, `i++`). Для атомарности — `synchronized`/`Lock`/`Atomic*`.
