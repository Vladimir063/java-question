...
TLS — это просто «новая версия SSL» с исправленными уязвимостями и улучшенной безопасностью.

⚡ Когда мы говорим «SSL-сертификат», фактически имеем в виду TLS-сертификат — термин просто прижился.

Оба протокола решают одну и ту же задачу:
Обеспечить безопасное шифрованное соединение между клиентом и сервером поверх TCP.
Они обеспечивают три ключевых свойства:
| Свойство                               | Что означает                                                                       |
| -------------------------------------- | ---------------------------------------------------------------------------------- |
| 🔐 **Шифрование (Encryption)**         | Данные передаются в зашифрованном виде, никто не может их перехватить и прочитать. |
| 🧾 **Аутентификация (Authentication)** | Сервер (и иногда клиент) подтверждает свою подлинность с помощью сертификата.      |
| 🧭 **Целостность (Integrity)**         | Проверяется, что данные не были изменены по пути.                                  |

# TLS/MTLS
TLS (Transport Layer Security) обеспечивает защищённый канал связи между клиентом и сервером, но в стандартной схеме только сервер предъявляет свой сертификат (например, *.example.com). Клиент проверяет этот сертификат, убеждается, что подключился к настоящему серверу, и дальше работает по зашифрованному каналу.
Таким образом, TLS защищает от подделки сервера и от перехвата данных, но не проверяет личность клиента.

mTLS (mutual TLS) добавляет второй этап — аутентификацию клиента. После того как сервер предъявил свой сертификат, он требует у клиента его собственный сертификат. Клиент тоже подписан доверенным центром сертификации, и сервер проверяет его подлинность. В результате обе стороны уверены, с кем говорят, и обмен полностью защищён.

| Характеристика               | TLS                               | mTLS                                                         |
| ---------------------------- | --------------------------------- | ------------------------------------------------------------ |
| Кто проходит проверку        | Только сервер                     | Сервер и клиент                                              |
| Проверка клиентской личности | Нет                               | Есть (по клиентскому сертификату)                            |
| Использование                | HTTPS, браузеры, открытые API     | Взаимодействие микросервисов, B2B-интеграции, внутренние API |
| Безопасность                 | Защита канала + доверие к серверу | Защита канала + взаимное доверие                             |
| Конфигурация                 | Только серверный сертификат       | Серверный + клиентские сертификаты, настройка trust store    |

Итог:
TLS обеспечивает конфиденциальность и целостность соединения, а mTLS добавляет к этому взаимную аутентификацию — обе стороны доказывают, что они доверенные участники.
