...
## Виртуализация
- **Что это:** запуск на одном физическом сервере нескольких **виртуальных машин** (VM).  
- **Как работает:** гипервизор (например, VMware ESXi, Hyper-V, VirtualBox) эмулирует аппаратное обеспечение. Каждая VM содержит **полную ОС** и приложения.  
- **Изоляция:** полная, на уровне ОС и ресурсов (каждая VM = своя ОС).  
- **Минусы:** «тяжёлые» — каждая VM тянет ядро ОС, тратит память и ресурсы на поддержку. Медленнее старт.  

## Контейнеризация
- **Что это:** запуск приложений в **контейнерах** — изолированных процессах в рамках одной ОС.  
- **Как работает:** контейнер использует ядро хост-ОС, но имеет свой файловый образ, сетевой стек, зависимости (через cgroups, namespaces в Linux). Docker, Podman, Kubernetes и др.  
- **Изоляция:** логическая (на уровне процессов и пространства имён), но ядро ОС общее.  
- **Плюсы:** лёгкие, быстро стартуют, мало оверхеда (нет отдельной ОС). Идеально для микросервисов.  

## Ключевые отличия

| Характеристика          | Виртуализация                     | Контейнеризация                 |
|--------------------------|-----------------------------------|---------------------------------|
| Уровень изоляции         | Полная (каждая VM — своя ОС)      | Логическая (ядро общее)         |
| Скорость запуска         | Секунды/минуты                   | Миллисекунды                    |
| Ресурсы                  | Тяжёлые (каждая VM несёт ОС)     | Лёгкие (только приложение + libs) |
| Гибкость                 | Можно ставить разные ОС           | Все контейнеры используют ядро хоста |
| Инструменты              | VMware, VirtualBox, KVM, Hyper-V | Docker, Podman, Kubernetes      |
| Use case                 | Монолитные приложения, legacy-системы, разные ОС на одном железе | Микросервисы, CI/CD, DevOps, быстрый деплой |

---

## Итог
- **Виртуализация** эмулирует **железо** и запускает полноценные ОС.  
- **Контейнеризация** изолирует **приложения** в рамках одной ОС.  
...
* `docker exec -it <container> <command>` — выполнение команды в работающем контейнере.
...
* `docker build -t <name> <path>` — создание образа из Dockerfile.
...
* `FROM` — задаёт базовый образ с Java.
* `WORKDIR` — устанавливает рабочую директорию.
* `COPY` — копирует готовый JAR-файл в контейнер.
* `EXPOSE` — открывает порт для связи с контейнером.
* `CMD` — задаёт команду запуска Java-приложения.
