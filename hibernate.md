...
## Источники (главные, для дальнейшего чтения)
- Hibernate User Guide — Performance Tuning / Fetching / Batching.
- Vlad Mihalcea — «How to batch insert and update statements with Hibernate».
- Baeldung — Batch insert/update with JPA/Hibernate.
- pgJDBC docs — курсоры и поведение fetchSize / autocommit.
- Franck Pachot — статьи про JDBC fetch size и Postgres.
- Статья про keyset pagination (why it's better чем OFFSET).
- CrunchyData / статьи про pg_stat_statements (как искать slow queries).

---

Если хочешь, могу:
- Сделать компактную «шпаргалку» (1–2 страницы) на собеседование.  
- Подготовить несколько задач/вопросов интервьюера с ответами (вопросы типа «как лечить N+1?», «почему OFFSET плох?» и т.д.).  
- Пройтись по твоему реальному коду / SQL и предложить конкретные оптимизации (для этого пришли код/микросхему/EXPLAIN).

Есть ли что-то из этого, что сделать первым?
